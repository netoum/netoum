...
constructor(el: HTMLElement, props: combobox.Props) {
 ...
 async performPagefindSearch(query: string): Promise<SearchItem[]> {
    ...
    try {
      ...
      for (const result of searchResults.results.slice(0, getNumber(this.el, "max-results" || 20))) { 
        try {
      ...
      onInputValueChange(...args) {
        ...
        if (!query) {
          ...
          self.performPagefindSearch(query).then((searchResults) => {
            self.options = searchResults.length > 0 ? searchResults : self.allItems;
            self.renderResults();
          });
          return;
        }
  ...
  renderResults() {
    ...
    if (this.options.length === 0) {
      ...
      noResult.textContent = getString(this.el, "no-result") || "No results found";
